...
function formatTotal(total, power) {
    ...
    let totalParts = getNumParts(total);
    let powerExp = '';
    ...
    if (totalParts.str.length > maxDigits) {
        ...
        let wholeNumDigits = totalParts.wholeNum.toString().length;
        if (wholeNumDigits > maxDigits) {
            // Convert to scientific notation
            let exponent = wholeNumDigits - 1;
            powerExp = "e" + exponent;
            total /= Math.pow(10, exponent);
            console.log('total2', total);

            // Round the decimal so that there's room for the exponent
            totalParts = getNumParts(total);
            console.log('decimalDigits', totalParts.decimalDigits);
            let decimalLength = maxDigits - totalParts.wholeNum.toString().length - powerExp.length;
            console.log('decimalLength', decimalLength);
            totalParts.decimalDigits = Math.round(totalParts.decimalDigits / Math.pow(10, totalParts.decimalDigits.toString().length - decimalLength));

            // If this rounding should result in incrementing the whole number part, then do so.
            ...
            console.log('rounded decimal digits', totalParts.decimalDigits);
...
function getNumParts(n) {
    ...
    if (containsDecimal(numParts.str)) {
        let parts_arr = numParts.str.split(/\./);
